...
function initializeOrders() {
    ...
    const detailsContainer = document.getElementById('orderDetailsPanel');
    ...
    // Load orders from Firestore if user is logged in
    if (userData && userData.course) {
        loadOrdersFromFirestore(userData.course);
        // Enable realtime updates
        setupRealtimeOrders(userData.course);
    }
    ...
    function renderOrders() {
        listContainer.innerHTML = orders.map((order, index) => `
            <div class="order-list-item" data-index="${index}">
                <h4>Order #${order.id}</h4>
                <p><i class="fas fa-user"></i> ${order.customer}</p>
                <p><i class="fas fa-dollar-sign"></i> $${order.total.toFixed(2)}</p>
                <p><i class="fas fa-check-circle"></i> ${order.paid ? 'Paid' : 'Unpaid'}</p>
            </div>
        `).join('');
        ...
        document.querySelectorAll('.order-list-item').forEach(item => {
            item.addEventListener('click', () => {
                // Remove active class from all
                document.querySelectorAll('.order-list-item').forEach(i => i.classList.remove('active'));
                // Add active to clicked
                item.classList.add('active');
                // Show details
                renderOrderDetails(item.dataset.index);
            });
        });
    }
    ...
    function renderOrderDetails(index) {
        ...
        detailsContainer.innerHTML = `
            <div class="order-detail-header">
                ...
                <div class="order-detail-meta">
                    ...
                    <span><i class="fas fa-dollar-sign"></i> $${order.total.toFixed(2)}</span>
                    <span><i class="fas fa-check-circle"></i> ${order.paid ? 'Paid' : 'Unpaid'}</span>
                </div>
                <div class="detail-actions">
                    <button class="btn-view-comanda" data-index="${index}">
                        <i class="fas fa-receipt"></i> View Comanda
                    </button>
                    <button class="btn-edit-order" data-index="${index}">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                </div>
            </div>
        ...
        // Add click listeners
        document.querySelector('.btn-view-comanda')?.addEventListener('click', (e) => {
            e.stopPropagation();
            viewComanda(index);
        });

        document.querySelector('.btn-edit-order')?.addEventListener('click', (e) => {
            e.stopPropagation();
            openEditModal(index);
        });
    }
    ...
    function openEditModal(index) {
        ...
        document.getElementById('editOrderPaid').checked = order.paid;
    ...
    // VIEW COMANDA
    // -------------------------
    function viewComanda(index) {
        const order = orders[index];
        if (!order) return;

        const comandaContent = document.getElementById('comandaContent');
        comandaContent.innerHTML = `
            <h3>Comanda - Order #${order.id}</h3>
            <p><strong>Customer:</strong> ${order.customer}</p>
            <p><strong>Items:</strong> ${order.items}</p>
            <p><strong>Total:</strong> ${order.paid ? '<span style="color: #4ade80;">PAGO</span>' : `$${order.total.toFixed(2)}`}</p>
            <p><strong>Status:</strong> ${order.paid ? 'Paid' : 'Unpaid'}</p>
    ...
    addForm?.addEventListener('submit', async e => {
        ...
        const order = {
            ...
            total: parseFloat(document.getElementById('newOrderTotal').value),
    ...
    editForm?.addEventListener('submit', e => {
        ...
        orders[index] = {
            ...orders[index],
            items: document.getElementById('editOrderItems').value.trim(),
            ...
            paid: document.getElementById('editOrderPaid').checked
        };
        ...
        if (confirm('Are you sure you want to delete this order?')) {
            ...
            detailsContainer.innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Select an order from the list to view details and manage payment status.</p>
                </div>
            `;
    ...
    // SAVE TO LOCALSTORAGE
    // -------------------------
    function saveOrders(autoSync = true) {
    ...
    // -------------------------
    async function loadOrdersFromFirestore(courseId) {
        try {
            const docRef = doc(db, "orders", courseId);
            const docSnap = await getDoc(docRef);

            if (docSnap.exists()) {
                orders = docSnap.data().orders || orders;
                saveOrders(); // Sync to localStorage
                renderOrders();
            } else {
                // If no Firestore data, save local data to Firestore
                saveOrdersToFirestore();
            }
        } catch (error) {
    ...
    async function saveOrdersToFirestore() {
        ...
        try {
            await setDoc(doc(db, "orders", userData.course), {
                orders: orders,
                lastUpdated: serverTimestamp()
            });
    ...
    // -------------------------
    // SETUP REALTIME ORDERS
    // -------------------------
    function setupRealtimeOrders(courseId) {
        const docRef = doc(db, "orders", courseId);
        onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                orders = docSnap.data().orders || orders;
                saveOrders(false); // Sync to localStorage without triggering another save
                renderOrders();
                // Re-render current order details if any
                const activeItem = document.querySelector('.order-list-item.active');
                if (activeItem) {
                    renderOrderDetails(activeItem.dataset.index);
                }
                console.log("Realtime update: Orders refreshed from cloud.");
            }
        }, (error) => {
            console.error("Realtime listener error:", error);
        });
